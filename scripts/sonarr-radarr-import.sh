...
# Sonarr/Radarr call this script again for upgrades and re-imports, and `ln -s`
# fails on an existing path — which, under `set -e`, would fail the whole import.
...
# Optional: log the successful operation. This must never fail the import — the
# symlink is already in place and Sonarr/Radarr treat a non-zero exit as a failed
# import, so a missing/read-only log directory is ignored.
IMPORT_LOG="${IMPORT_SCRIPT_LOG:-/config/log/import-script.log}"
if mkdir -p "$(dirname "$IMPORT_LOG")" 2>/dev/null; then
    echo "$(date): Successfully created symlink: $DEST -> $SOURCE" >> "$IMPORT_LOG" 2>/dev/null || true
fi
