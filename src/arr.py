...
def _is_hardlink_to_hdd(imported_path: str, hdd_path_norm: str) -> bool:
    ...
    try:
        ...
        # (including a single file inside a root folder), so compare it directly
        # rather than walking it as a directory.
        if os.path.isfile(hdd_path_norm):
            try:
                hdd_stat = os.stat(hdd_path_norm)
                if (imported_stat.st_dev == hdd_stat.st_dev and
                        imported_stat.st_ino == hdd_stat.st_ino and
                        imported_path != hdd_path_norm):
                    logger.debug(f"Found hardlink: {imported_path} <-> {hdd_path_norm}")
                    return True
